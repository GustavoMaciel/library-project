package br.edu.ufcg.virtus.library.core.repository;
...
public class SearchSpecification<M> implements Specification<M> {
    ...
    @Override
    public Predicate toPredicate(Root<M> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
    ...
    private List<Field> getValidFields(Root<M> root) {
        List<String> permittedTypes = Arrays.asList("string", "date", "integer", "long", "boolean", "instant");
        List<Field> fields = new ArrayList<>();
        for (Field field: root.getJavaType().getDeclaredFields()) {
            if (permittedTypes.contains(field.getType().getSimpleName().toLowerCase())) {
                fields.add(field);
            }
        }
        return fields;
